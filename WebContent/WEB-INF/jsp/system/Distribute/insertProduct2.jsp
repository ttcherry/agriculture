	...
	<form class="layui-form" id="form1">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="text" name="product_name" required lay-verify="required" placeholder="请输入产品名称" autocomplete="off" class="name layui-input" id="one">
                </div>
            </div>
       		<div class="layui-form-item">
              <label class="layui-form-label">服务关注重点</label>
              <div class="layui-input-block">
                <input type="text" name="important" required lay-verify="required" placeholder="请输入服务关注重点" autocomplete="off" class="wded layui-input" id="two" >
              </div>
            </div>
            <div class="layui-input-block">
...
layui.use(['form', 'layedit', 'laydate','layer','jquery'], function(){
	  ...
	  $('.add').on('click', function() {
		 ...
		 if(one.length<1&&two.length<1){
			 layer.msg('添加失败，请输入产品名称和服务关注重点');
			return false;
		 }
		 if(two.length<1&&one.length>1){
			 layer.msg('添加失败，请输入服务关注重点');
			 return false;
		 }
		 if(one.length<1&&two.length>1){
			 layer.msg('添加失败，请输入请输入产品名称');
		 	return false;
		 }
				$.ajax({
		        cache: false,
		        type: "POST",
		        url:"${pageContext.request.contextPath }/insertProduct.action",
		        ...
		        success: function(data) {
		        	if(data=="success"){
		        		...
		        		layer.msg('添加失败，你输入的产品名称重复，请重新输入');
		        	}
		        },
		        error: function(data) {
